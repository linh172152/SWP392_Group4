...
const DriverLayout: React.FC<DriverLayoutProps> = ({ user, onLogout, children }) => {
          ...
          <div className="fixed top-0 left-0 bottom-0 w-64 glass-card border-0 border-r border-white/20 dark:border-slate-700/50 shadow-2xl">
            <div className="flex items-center justify-between h-16 px-4 border-b border-white/20 dark:border-slate-700/50">
              <div className="flex items-center space-x-2">
                ...
                <span className="font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">EVSwap Tài xế</span>
              </div>
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center px-4 py-3 text-sm font-medium transition-all duration-300 ${
                      isActive
                        ? 'bg-gradient-to-r from-blue-500/20 to-purple-500/20 text-blue-700 dark:text-purple-400 border-r-2 border-blue-600 dark:border-purple-400 shadow-lg'
                        : 'text-slate-600 dark:text-slate-300 hover:text-blue-600 dark:hover:text-purple-400 hover:bg-blue-50/50 dark:hover:bg-purple-500/10'
                    }`}
                  >
        ...
        <div className="flex flex-col flex-grow glass-card border-0 border-r border-white/20 dark:border-slate-700/50 shadow-2xl">
          ...
          <nav className="mt-4 flex-1">
            {navigation.map((item) => {
              ...
              return (
                ...
                >
                  <Icon className={`mr-3 h-5 w-5 group-hover:scale-110 transition-transform duration-300 ${isActive ? 'text-blue-600 dark:text-purple-400' : ''}`} />
                  {item.name}
                </Link>
              );
            })}
          ...
          <div className="flex-shrink-0 p-4 border-t border-white/20 dark:border-slate-700/50">
            <div className="flex items-center space-x-3 mb-3">
              <Avatar className="ring-2 ring-blue-500/20 dark:ring-purple-500/20">
      ...
      <div className="lg:pl-64 relative z-10">
        ...
        <div className="sticky top-0 z-30 lg:hidden">
          <div className="flex items-center justify-between h-16 px-4 glass-card border-0 border-b border-white/20 dark:border-slate-700/50 shadow-lg">
            <Button variant="ghost" size="sm" onClick={() => setMobileMenuOpen(true)} className="hover:bg-blue-50/50 dark:hover:bg-purple-500/10">
              <Menu className="h-5 w-5" />
            </Button>
            <div className="flex items-center space-x-2">
              <div className="p-1.5 gradient-primary rounded-md">
                <Zap className="h-4 w-4 text-white" />
              </div>
              <span className="font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">EVSwap</span>
            </div>
            <div className="flex items-center space-x-2">
              <ThemeToggle />
              <Avatar className="ring-2 ring-blue-500/20 dark:ring-purple-500/20">
