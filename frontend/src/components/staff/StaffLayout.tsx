...
const StaffLayout: React.FC<StaffLayoutProps> = ({ user, onLogout, children }) => {
  const location = useLocation();
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-green-50 to-emerald-100 dark:from-slate-900 dark:via-green-900/20 dark:to-emerald-900/20 transition-all duration-500">
          ...
          <div className="fixed top-0 left-0 bottom-0 w-64 glass-card border-0 border-r border-white/20 dark:border-slate-700/50 shadow-2xl">
            <div className="flex items-center justify-between h-16 px-4 border-b border-white/20 dark:border-slate-700/50">
              <div className="flex items-center space-x-2">
                <div className="p-2 bg-gradient-to-r from-green-500 to-emerald-500 rounded-lg glow">
                  <Zap className="h-4 w-4 text-white" />
                </div>
                <span className="font-bold bg-gradient-to-r from-green-600 to-emerald-600 bg-clip-text text-transparent">EVSwap Nhân viên</span>
              </div>
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center px-4 py-3 text-sm font-medium transition-all duration-300 ${
                      isActive
                        ? 'bg-gradient-to-r from-green-500/20 to-emerald-500/20 text-green-700 dark:text-emerald-400 border-r-2 border-green-600 dark:border-emerald-400 shadow-lg'
                        : 'text-slate-600 dark:text-slate-300 hover:text-green-600 dark:hover:text-emerald-400 hover:bg-green-50/50 dark:hover:bg-emerald-500/10'
                    }`}
                  >
          ...
          <nav className="mt-4 flex-1">
            {navigation.map((item) => {
              ...
              return (
                ...
                >
                  <Icon className={`mr-3 h-5 w-5 group-hover:scale-110 transition-transform duration-300 ${isActive ? 'text-green-600 dark:text-emerald-400' : ''}`} />
                  {item.name}
                </Link>
              );
            })}
      ...
      <div className="lg:pl-64 relative z-10">
        ...
        <div className="sticky top-0 z-30 lg:hidden">
          <div className="flex items-center justify-between h-16 px-4 glass-card border-0 border-b border-white/20 dark:border-slate-700/50 shadow-lg">
            <Button variant="ghost" size="sm" onClick={() => setMobileMenuOpen(true)} className="hover:bg-green-50/50 dark:hover:bg-emerald-500/10">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="font-bold bg-gradient-to-r from-green-600 to-emerald-600 bg-clip-text text-transparent">EVSwap</span>
            </div>
            <div className="flex items-center space-x-2">
              <ThemeToggle />
              <NotificationBell />
