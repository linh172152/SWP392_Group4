...
const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      const response = await fetch(API_ENDPOINTS.GOOGLE.AUTH);
      const data = await response.json();
          ...
          <TabsContent value="login" className="space-y-4">
            ...
            <div className="relative my-6">
              ...
              <div className="relative flex justify-center text-xs uppercase">
                <span className="bg-white dark:bg-slate-800 px-2 text-slate-500">Hoặc</span>
              </div>
            </div>
            ...
            <Button
              type="button"
              variant="outline"
              className="w-full glass border-slate-300 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700"
